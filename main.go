...
func main() {
	...
	fmt.Println("--<-@  HREF examples are created inside strs page.....")
	...
	htmlstart2 := "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>QRCoder</title><style>body { background-color: black; } table { width: 100%; border-collapse: collapse; } th, td { padding: 15px; overflow: hidden; max-width: 100px; text-align: center; border: 1px solid yellow; color: yellow; width: 200px; left: 0;} h1 {color: yellow; text-align: center;} div {color: yellow; font-size: 30px; text-align: right;} a {color: yellow; font-size: 30px; text-align: center;}</style></head><div >Follow: <a href=https://t.me/zeropticum>[tg]</a><a href=https://zeropticum.org>[zeroticum.org]</a></div><h1>QRCoder ▩ Zeropticum</h1><body>"
	htmlend2 := "</body></html>"

	addsrts := make([]string, 0)

	for i := range rezstrikes {

		addsrts = append(addsrts, "<a href="+rezstrikes[i]+">"+rezstrikes[i]+"</a><br>")
	...
	rezaddstr := strings.Join(addsrts, "") //

	html2 := htmlstart2 + rezaddstr + htmlend2

	file2, err := os.Create("strs.html")
	if err != nil {
		log.Fatal("Error File strs.html Create")
	}
	defer file2.Close()

	_, err = file2.WriteString(html2)

	fmt.Println("--<-@  DONE! Look:   qrs.html, strs.html")
	fmt.Println("--<-@  With Love from ZL...")
